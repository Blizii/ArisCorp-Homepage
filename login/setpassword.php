...
<body>
    ...
    if(isset($_GET["token"])){
        require("mysql.php");
        $stmt = $mysql->prepare("SELECT * FROM MemberAccounts WHERE TOKEN = :token"); //Username überprüfen
        ...
        if($count != 0){
            if(isset($_POST["submit"])){
                if($_POST["pw1"] == $_POST["pw2"]){
                    ...
                    $stmt = $mysql->prepare("UPDATE MemberAccounts SET PASSWORD = :pw, TOKEN = null WHERE TOKEN = :token");
